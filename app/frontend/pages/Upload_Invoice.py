...
st.write(
    "Upload a photo of an invoice — RetailMind will read it "
        ...
        if response.status_code == 200:
            ...
            if "error" in result:
                ...
                if result.get("needs_profile_questions"):
                    ...
                    st.session_state["new_customer_id"] = (
                        result["customer_id"]
                    )
...
if "new_customer_id" in st.session_state:
    ...
    with st.form("customer_profile_form"):
        ...
        payment_mode = st.selectbox(
            ...
            ["Cash", "UPI", "Credit", "Mixed"]
        )

        preference_tier = st.selectbox(
            "Preferred product range",
            ["Budget", "Mid-range", "Premium", "Not sure"]
        )

        buying_behavior = st.selectbox(
            "Buying behavior",
            [
                "Planned purchases",
                "Impulse purchases",
